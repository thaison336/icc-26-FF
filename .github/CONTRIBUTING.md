...
Every new contributor must sign the CLA document before their contribution can be accepted. It can be signed as an individual or on behalf of a company.  
Signatures have a 6-month expiration period.

This repository does not include a CLA automation workflow; follow your organization’s CLA process and the [CLA.md](../CLA.md) pointer if applicable.
...
4. **Evaluate the Action Workflow Results**  
   This repository includes the following GitHub Actions workflow:
   - **[Secret Scanner](workflows/04-TruffleHog-Security-Scan.yml)**: Runs the TruffleHog security scanner on pushes to selected branches to look for verified leaked secrets (API keys, tokens, etc.). Other quality gates (build, formatting) are your responsibility unless you add workflows or use organization-level checks.

### As a Reviewer

What to consider when reviewing a Pull Request:

- All required checks must pass successfully (including any checks configured for this repository).
