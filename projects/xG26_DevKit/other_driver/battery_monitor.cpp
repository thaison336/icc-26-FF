 ...
 * @brief SomniGuard Battery Monitor — Trình điều khiển IADC0 đọc điện áp pin
 * qua PD02
 */
...
void battery_monitor_init(void)
{
  ...
  // 1. Cấp clock cho GPIO và IADC0
  CMU_ClockEnable(cmuClock_GPIO, true);
  CMU_ClockEnable(cmuClock_IADC0, true);
  CMU_ClockSelectSet(cmuClock_IADCCLK, cmuSelect_FSRCO);

  // 2. Chuyển chân PD02 sang chế độ Disabled (chế độ Analog chuẩn của Silicon Labs)
  GPIO_PinModeSet(BATT_ADC_PORT, BATT_ADC_PIN, gpioModeDisabled, 0);

  // 3. QUAN TRỌNG: Cấp phép kết nối chân chẵn Port C/D (PD02) vào bộ chuyển đổi IADC0
  ...
  allConfigs.configs[0].adcClkPrescale = IADC_calcAdcClkPrescale(
      IADC0, 10000000UL, 0, iadcCfgModeNormal, init.srcClkPrescale);

  IADC_InitSingle_t initSingle = IADC_INITSINGLE_DEFAULT;
  IADC_SingleInput_t input = IADC_SINGLEINPUT_DEFAULT;
  input.posInput = iadcPosInputPortDPin2; // PD02
  ...
  printf("[BATT] Battery monitor initialized successfully with CDBUSALLOC!\r\n");
}
...
void battery_monitor_sample(void)
{
  ...
  // Tính điện áp đo được tại chân PD02 (mV)
  // Vadc = raw_code * Vref_full_scale / Resolution (2420 mV / 4096)
  ...
  // printf("[BATT DEBUG] Raw: %4lu | Vadc: %4lu mV | Vbat: %4lu mV\r\n",
  //        raw_code, vadc_mv, vbat_sample_mv);
